...
import { BLANKET_CONSENT_TEXT_ES } from '@/lib/constants';

// ---------------------------------------------------------------------------
// Brand colors — uses a deep blue accent to visually distinguish full-query
// documents from limited-query (gold accent) documents.
// ---------------------------------------------------------------------------
...
const styles = StyleSheet.create({
  ...
  // Blue accent bar (distinguishes full-query from limited-query gold bar)
  accentBar: {
    height: 3,
...
function ConsentHaulLogo({ size = 28 }: { size?: number }) {
  return (
    <Svg width={size} height={size} viewBox="0 0 48 48">
      ...
      <Circle cx="35.5" cy="35.5" r="2.6" fill={brand.accent} />
    </Svg>
  );
}
...
export interface BlanketQueryEsProps {
  consent: {
    id: string;
    consent_type: string;
...
export function BlanketQueryEs({
  consent,
  driver,
  organization,
  signatureDataUrl,
  ...
  const consentBody = BLANKET_CONSENT_TEXT_ES.body(
    driver.name,
    organization.name,
    ...
    <Document>
      <Page size="LETTER" style={styles.page}>
        ...
        <View style={styles.badge}>
          <Text style={styles.badgeText}>Consentimiento de Consulta Completa (General)</Text>
        </View>
        ...
        <View style={styles.acknowledgmentBox}>
          <Text style={styles.acknowledgmentText}>
            {BLANKET_CONSENT_TEXT_ES.acknowledgment}
          </Text>
        </View>
