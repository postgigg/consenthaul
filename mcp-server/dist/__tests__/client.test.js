...
describe('ConsentHaulClient', () => {
    ...
    it('injects Bearer auth header', async () => {
        ...
        expect(req[1]?.headers).toEqual(expect.objectContaining({ Authorization: 'Bearer ch_test_key123' }));
    });
    ...
    it('extracts .error from error response body', async () => {
        vi.spyOn(globalThis, 'fetch').mockResolvedValue(new Response(JSON.stringify({ error: 'Not Found', message: 'Driver not found.' }), { status: 404 }));
