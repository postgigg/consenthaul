...
if (!apiKey) {
    console.error("Error: CONSENTHAUL_API_KEY environment variable is required.\n" +
        "Get your API key from https://app.consenthaul.com/api-docs");
    process.exit(1);
}
const baseUrl = process.env.CONSENTHAUL_BASE_URL || "https://app.consenthaul.com";
...
for (const [name, tool] of Object.entries(allTools)) {
    server.tool(name, tool.description, tool.inputSchema.shape, async (input) => {
        try {
            ...
            const isApiError = typeof err === "object" &&
                err !== null &&
                "status" in err &&
                "message" in err;
